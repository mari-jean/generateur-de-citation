...
let dernier    = 0;// Indice de la dernière citation affichée
let nombreAleatoire = 0;// Nombre aléatoire pour sélectionner une citation

// Tableau de citations avec chaque citation sous la forme [citation, auteur]
let citations = [
...
// Générer un nombre entier aléatoire entre 0 (inclus) et max (exclus)
function genererNombreEntier(max) {
    return Math.floor(Math.random()*Math.floor(max));
}

// Ajouter un écouteur d'événement au bouton "Nouveau"
nouveau.addEventListener("click",()=>{
        do{
            nombreAleatoire = genererNombreEntier(citations.length);
            
            
        }while (nombreAleatoire === dernier);// Vérifier que la citation générée n'est pas la même que la précédente
        ...
        // Mettre à jour l'indice de la dernière citation affichée
